...
thread_sleep(int64_t ticks){
  ...
  list_push_back(&sleeping_threads, &curr->elem); 
  //list_insert_ordered (&sleeping_threads, &curr->elem, thread_compare_wakeup_tick, NULL);
  //set_wakeup_tick (ticks); 
...
/*
bool
thread_compare_wakeup_tick (const struct list_elem *first, const struct list_elem *second, void *aux UNUSED){
  return list_entry(first, struct thread, elem)->wakeup_tick < list_entry(second, struct thread, elem)->wakeup_tick;
}

void
set_wakeup_tick (int64_t ticks){
  if (awake_tick > ticks) awake_tick = ticks;
}
int64_t
...
thread_awake(int64_t ticks){
  ...
  while (!list_empty(&sleeping_threads) && ticks >= sleeping_thread->wakeup_tick) {
    e = list_remove(e);
  ...
  if (list_empty(&sleeping_threads)) awake_tick = INT64_MAX;
  else awake_tick = sleeping_thread->wakeup_tick;
}
*/
...
bool
lock_compare_priority (const struct list_elem *first, const struct list_elem *second, void *aux UNUSED){
  struct lock *first_lock = list_entry(first, struct lock, elem);
  struct lock *second_lock = list_entry(second, struct lock, elem);

  if(list_empty(&first_lock->semaphore.waiters))
    return false;

  if(list_empty(&second_lock->semaphore.waiters))
    return true;

  struct list_elem *first_elem;
  struct list_elem *second_elem;
  first_elem = list_front(&first_lock->semaphore.waiters);
  second_elem = list_front(&second_lock->semaphore.waiters);

  return list_entry(first_elem, struct thread, elem)->priority > list_entry(second_elem, struct thread, elem)->priority;
}
...
thread_exit (void) 
{
  ...
  //printf("thread_exit : 1111\n");
  sema_up (&thread_current()->child_exit);
  //printf("thread_exit : 2222\n");
...
thread_set_priority (int new_priority) 
{
  struct thread * curr= thread_current();
  /*
  if (thread_current()->donated_times >= 1){
    curr->old_priority = new_priority;
  }
  else {
    thread_current ()->priority = new_priority;
  }
  if ( curr->priority < thread_max_priority() ){
    thread_yield();
  }*/
...
/*
void
thread_set_priority_advanced (int new_priority, struct thread * other_thread){
  other_thread->priority = new_priority;
  list_sort(&ready_list, thread_compare_priority,NULL);
  if (thread_current ()->priority < other_thread->priority){
    thread_yield();
  }
}
int
